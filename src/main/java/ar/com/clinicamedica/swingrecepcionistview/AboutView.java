...
import ar.com.clinicamedica.values.Links;

public class AboutView extends JPanel {

	private static final String FER_LINKEDIN_URL = "https://ar.linkedin.com/in/fernando-corinaldesi-2a839b175";
	private static final String FER_GITHUB_URL = "https://github.com/portfolio-box";
	...
	public AboutView(String select) {
		
		if(select.equals("main")){
			this.path=Links.ABOUTRECEPTION;
			
		}
		if(select.equals("medic")){
			this.path=Links.ABOUTMEDIC;
			
		}
		if(select.equals("admin")){
			this.path=Links.ABOUTADMIN;
	...
	private JPanel createNorthPane() {
		...
		JLabel labelTitle = new JLabel(new ImageIcon(getClass().getResource(path)));
	...
	private JPanel whatsapp() {
		JPanel panel = new JPanel();
		JLabel label = new JLabel("Whatsapp : 11-22829-6699");
		panel.add(label);
		panel.setBackground(Color.WHITE);
		return panel;
	}
	...
	private JPanel version() {
		JPanel panel = new JPanel();
		JLabel label = new JLabel("Version del software : 1.0.0");
		panel.add(label);
		panel.setBackground(Color.WHITE);
		return panel;
	}
	...
	private JPanel db() {
		JPanel panel = new JPanel();
		JLabel label = new JLabel("Base de datos : MySQL");
		panel.add(label);
		panel.setBackground(Color.WHITE);
		return panel;
	}
